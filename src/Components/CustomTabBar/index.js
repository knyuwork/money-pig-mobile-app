...
class CustomTabBar extends Component<Props> {
  ...
  renderIcon = route => {
    const tintColor = '#fff'
    const { activeTab, tabBarIndex } = this.props
        ...
        >
          ...
          {tabs.map((tab, idx) => (
            ...
            >
              <TouchableWithoutFeedback
                key={idx}
                onPress={() => this.onTabPress(tab, idx)}
              >
                <View>{this.renderIcon(tab)}</View>
              </TouchableWithoutFeedback>
            </View>
          ))}
          <View style={{ width: SCREEN_WIDTH / 4 + 16, height: 16 }} />
