...
const dashboardReducer = handleActions(
  {
    [ACTION_TYPES.SAVE_LOCAL_OCTOPUS_RECORD]: (
      ...
      const updatedTotalAmount =
        parseFloat(state.get('totalAmount')) + parseFloat(record.amount)
    ...
    [ACTION_TYPES.UPDATE_LOCAL_HISTORY]: (state, { payload: { history } }) =>
      state.set('history', fromJS(history)),
    [ACTION_TYPES.SET_TOTAL_AMOUNT]: (state, { payload: { amount } }) =>
